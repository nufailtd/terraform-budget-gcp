...
output "k8s_service_account_name" {
  ...
  depends_on  = [google_project_iam_member.workload_identity_sa_bindings]
}
...
output "annotate_sa" {
  ...
  value       = "kubectl annotate --overwrite sa -n ${local.output_k8s_namespace} ${local.k8s_given_name} iam.gke.io/gcp-service-account=${local.gcp_sa_email}"
}
